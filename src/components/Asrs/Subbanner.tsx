...
import { subbannerConfig2 } from "../../config/subbannerConfig2";
import { ASSET_PREFIX } from "../../../config";

gsap.registerPlugin(ScrollTrigger);

interface SubbannerProps {
  bannerKey: keyof typeof subbannerConfig2;
}

export default function Subbanner({ bannerKey }: SubbannerProps) {
  const banner = subbannerConfig2[bannerKey];
      ...
      <div className="absolute inset-0 z-[2] flex flex-col justify-end px-[20px] sm:px-[40px] py-[30px] text-white">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            {banner.description && (
              <p className="text-[16px] sm:text-[18px] font-light max-w-[50%] mb-[25px] hidden md:block">
                {banner.description}
              </p>
            )}
            {banner.link && (
...
function BannerLink({
  ...
  return (
    <Link
      ...
      <div
        className="relative p-[12.5px] rounded-full flex items-center justify-center
      ...
      <span className="relative z-[99] opacity-80 group-hover:opacity-100 group-hover:scale-[1] transition duration-300 ease-in-out pr-[15px] ">
        {children}
      </span>
    </Link>
  );
