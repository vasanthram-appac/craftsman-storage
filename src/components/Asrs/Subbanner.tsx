...
export default function Subbanner({ bannerKey }: SubbannerProps) {
  ...
  return (
    <section
      ref={containerRef}
      className="relative overflow-hidden rounded-[25px] sm:rounded-[40px] h-[350px] md:h-[400px] xl:h-[500px]"
    >
      {/* Background Media */}
      ...
      <div className="absolute inset-0 z-[2] flex flex-col justify-end px-[20px] sm:px-[40px] py-[30px] text-white">
        <AnimatePresence mode="wait">
          ...
          >
            <h1 className="text-[28px] sm:text-[34px] md:text-[42px] xl:text-[48px] font-semibold leading-tight mb-[15px]">
              {banner.subtitle}
            </h1>
            {banner.description && (
...
function BannerLink({
  ...
  return (
    <Link
      href={href}
      className="inline-flex items-center gap-[10px] text-[16px] font-normal hover:opacity-80 transition"
    >
      <div className="bg-[#000] p-[10px] rounded-full">
        <ArrowIcon />
      </div>
      {children}
    </Link>
